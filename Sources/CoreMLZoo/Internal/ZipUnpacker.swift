...
enum ZipUnpacker {
    ...
    static func unpack(archive: URL, to destination: URL) throws {
        let raw: Data
        do { raw = try Data(contentsOf: archive, options: [.mappedIfSafe]) }
        catch { throw UnzipError.io(error.localizedDescription) }
        let bytes = [UInt8](raw)
        ...
        for entry in entries {
            ...
            switch entry.method {
            case 0:  decompressed = Data(compressed)
            case 8:  decompressed = try inflate(compressed, uncompressedSize: Int(entry.uncompressedSize))
            default: throw UnzipError.unsupported("compression method \(entry.method)")
            }
    ...
    private static func parseCentralDirectory(bytes: [UInt8]) throws -> [Entry] {
        ...
        guard let eocd = eocdOffset else { throw UnzipError.invalidArchive("no EOCD") }
    ...
    private static func readLocalFile(bytes: [UInt8], entry: Entry) throws -> [UInt8] {
        ...
        guard dataEnd <= bytes.count else { throw UnzipError.invalidArchive("LFH overflow") }
    ...
    private static func inflate(_ input: [UInt8], uncompressedSize: Int) throws -> Data {
        ...
        if written == 0 { throw UnzipError.io("decompression failed") }
